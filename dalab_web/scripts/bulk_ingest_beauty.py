...
IMAGE_DIR = r'C:\Users\user\OneDrive\图片\dalab-categories-products\beauty\beauty-improved-image'
DB_PATH = 'prisma/dev.db'
STORE_ID = 'cmkvifmt40002tks8r6z5o5hn'  # Dalab
CATEGORY_ID = 'cat_daaffb78'  # Beauty
...
def upload_to_supabase(image_path: str, product_id: str) -> str:
    ...
    try:
        ...
        storage_path = f'beauty/{product_id}{file_ext}'
...
def create_product(conn, product_id: str, title: str, image_url: str):
    ...
    try:
        ...
        """, (product_id, title, f"Beauty product - {title}", image_url, STORE_ID, CATEGORY_ID, 'ACTIVE', 0))
        ...
        sku = f"BEAUTY-{uuid.uuid4().hex[:12]}"  # Use full UUID for uniqueness
        cursor.execute("""
...
def main():
    print("🌸 Starting Bulk Beauty Product Ingestion...\n")
    ...
    for idx, filename in enumerate(image_files, 1):
        product_id = f"beauty_{uuid.uuid4().hex[:8]}"
        title = f"Beauty Product {idx}"
