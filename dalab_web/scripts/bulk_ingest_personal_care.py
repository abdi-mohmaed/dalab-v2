...
IMAGE_DIR = r'C:\Users\user\OneDrive\图片\dalab-categories-products\personal care\personalcare-improved-images'
DB_PATH = 'prisma/dev.db'
STORE_ID = 'cmkvifmt40002tks8r6z5o5hn'  # Dalab
CATEGORY_ID = 'cat_a9c4fd5e'  # Personal Care
...
def upload_to_supabase(image_path: str, product_id: str) -> str:
    ...
    try:
        ...
        storage_path = f'personal-care/{product_id}{file_ext}'
...
def create_product(conn, product_id: str, title: str, image_url: str):
    ...
    try:
        ...
        """, (product_id, title, f"Personal care product - {title}", image_url, STORE_ID, CATEGORY_ID, 'ACTIVE', 0))
...
def main():
    print("Starting Bulk Personal Care Product Ingestion...\n")
    ...
    for idx, filename in enumerate(image_files, 1):
        ...
        if title.startswith('Whatsapp image'):
            title = f"Personal Care Product {idx}"
            
        # Check if product already exists
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM Product WHERE title = ? AND categoryId = ?", (title, CATEGORY_ID))
        if cursor.fetchone():
            print(f"[{idx}/{len(image_files)}] Skipping: {title} (Already exists)")
            skipped_count += 1
            continue

        product_id = f"care_{uuid.uuid4().hex[:8]}"
    ...
    print(f"   Skipped: {skipped_count}/{len(image_files)}")
